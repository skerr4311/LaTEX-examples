...
We can then close the two under union:
 $$(A \cap B) \cup (A \cap \overline{B}) = \emptyset$$
and use the following Turing machine $M^{*}$ to show that the set of Turing decidable languages are closed under both $\bigodot$ and $\bigotimes$:
\newpage
Let $A^{'}$ be the DFA that accepts the language $A$. Let $B^{'}$ be the DFA that accepts the language $B$. 
...
\item Let $D^{*}$ be the DFA that recognises $(A \cap B)$. Use $T$ on $<D^{*}>$. If $T$ accepts $accept$. If $T$ rejects, go to 3. 
\item Take a string $w$ such that $w \in \{0,1\}^{*}$ and use Turing machine $N$ on $w$. If $N$ accepts, $accept$. If $N$ rejects, $reject$.
\end{enumerate}
As we can see, machine $M^{*}$ can only $accept$ or $reject$ the two languages. This is because we use $T$ from theorem 10 which decides a DFA input. So $T$ will always halt in an accept or reject state. We also make use of Theorem 3. iv where Turing machine $N$ decides concatenation of two decidable languages. $N$ is also decidable because $N$ will always halt in an accept or reject state. We already know the languages $A$ and $B$ are Turing-decidable and we can see that $M^{*}$ is also decidable. Therefore, $M^{*}$ decides: $$\bigodot (A, B) = \{w \in \{0,1\}^{*} | (w \in A \; \& \; w \in B)\; or \; w \in A  \circ B \}$$
$$\bigotimes (A, B) = \{w \in \{0,1\}^{*} | (w \in A \; \& \; w \notin B)\; or \; w \in A  \circ B \}$$
closed under union.
