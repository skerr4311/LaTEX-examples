...
\\
\textbf{2. iii} \\
We start by assuming that there is a turing machine $M$ that decides $L_1$. We can then construct a turing machine $M^{'}$ that decides $\overline{L_1}$ in the following way: \\
$M^{'}$ = "On input $w$": 
\begin{enumerate}
\item Simulate $M$ on $w$.
\item $Accept$ if $M$ rejects, $reject$ if $M$ accepts.
\end{enumerate} 
$M^{'}$ always halts because $M$ always halts. $M^{'}$ gives the correct result, because if $w \in L_1$ $M$ will accept and so $M^{'}$ will reject, and if $w \notin L_1$, $M$ will reject in which case $M^{'}$ will accept. \\
This means that $M^{'}$ decides $\overline{L_1}$ and the language is decidable. \\
\\
Notice that $\overline{L_1}$ $\subseteq$ $L_2 \cup \overline{L_2}$ This is because we know that:
\begin{enumerate}
\item $\overline{L_1} = \{w \in \{0,1\}^{*}|$ w does not contain the same number of $1^s$ and $0^s$ $\}$
\item $L_2 = \{w \in \{0,1\}^{*}|$ w contains more $1^s$ than $0^s$ $\}$
\item $\overline{L_2} = \{w \in \{0,1\}^{*}|$ w contains less $1^s$ than $0^s$ $\}$
\end{enumerate}   
This means that $M^{'}$ decides $L_2 \cup \overline{L_2}$. This means that $L_2$ and $\overline{L_2}$ are both decidable languages and there must exist a TM that can decide them making $L_2$ Turing-decidable.
\newpage
...
Proof: Firstly, we know that any NFA can be replicated by a DFA so let $Q = \{ <A, B> |$ A and B are DFAs over some alphabet $\Sigma$ and $\emptyset \sqsubseteq L(A)$ and $L(A) \sqsubseteq L(B)\}$. \\
Secondly, by definition, we know the empty set is a proper subset of any nonempty set. So, we know that $L(A)$ and $L(B)$ are not empty sets. \\
Again, by definition, we know that $L(B)$ is larger than $L(A)$ because $L(A)$ is a subset of $L(B)$ and by the definition of 'a proper subset' $L(A) \neq L(B)$.\\
$L(A) \sqsubseteq L(B)$ iff $L(A) \subset L(B)$ and $L(A) \neq L(B)$\\
iff $(\overline{L(A)} \cap L(B)) \neq \emptyset$ and $(L(A) \cap L(B)) \neq \emptyset$ \\
iff $(\overline{L(A)} \cap L(B)) \cup (L(A) \cap L(B)) \neq \emptyset$ \\
\\
From the proofs of the closure of the class of regular languages under intersection, complement and union, given DFAs $A$ and $B$ we can construct a DFA C which recognises 
 $(\overline{L(A)} \cap L(B)) \cup (L(A) \cap L(B)) \neq \emptyset$. \\
This construction can be carried out by a TM. Once we have such a C, we can test to see if $L(A) \neq L(B)$ by testing to see if $L(C) \neq \emptyset$, using the TM $C^*$. \\
\\
$C^*$ = 'On input $<A$,$B>$, where A and B are DFAs:',
\begin{enumerate}
\item Construct a DFA C such that: \\
		$L(C) = (\overline{L(A)} \cap L(B)) \cup (L(A) \cap L(B))$\\
On input $<C>$:
\item Mark the start state of $C$.
\item Mark any state that has a transition coming into if from a marked state. If no new states get markedf this way, got to 3. Else, go to 2 (ie, repeat).
\item If no accept state is marked, reject. Otherwise, accept.
\end{enumerate}
